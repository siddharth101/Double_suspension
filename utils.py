...
def get_wire_dir(body1, body2, points_1='top',
                     points_2='top', suspension_body=None):

    paths = get_linpaths(body1, body2, points_1=points_1,
                     points_2=points_2, suspension_body=suspension_body)['paths']

    S = suspension_body

    dirs = [i.attachments[0].pos_from(i.attachments[1]).express(S.global_frame).normalize() for i in paths]

    return dirs

def get_linpaths(body1, body2, points_1='top',
                     points_2='top', suspension_body=None):
    ...
    print(body2_points)
    S = suspension_body
    linpaths = {'paths':[LinearPathway(i,j) for i, j in list(zip(body1_points, body2_points))]} | {'bodies':[body1.body, body2.body]}
    
    
    #linpaths.bodies = [body1, body2]
...
def apply_force(body, forcedict, global_frame, bottom=True):
    ...
    for i in range(4):
        body.body.apply_force(top_path[i].to_loads(-top_tensions[i], frame=global_frame)[1][1], point=top_points[i])
    ...
    if bottom:
        ...
        for i in range(4):
            body.body.apply_force(bottom_path[i].to_loads(-bottom_tension[i], frame=global_frame)[0][1], point=bottom_points[i])
    else:
        pass
