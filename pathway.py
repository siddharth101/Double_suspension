...
class LinearPathway(PathwayBase):
    ...
    def length(self, frame=None):
       ...
       # print("Returning this for {}".format(frame))
        return _point_pair_length(*self.attachments, frame)
    ...
    def to_loads(self, force, frame=None):
        """Loads required by the equations of motion method classes.
        ...
        """
        relative_position = _point_pair_relative_position(*self.attachments, frame)
        length = _point_pair_length(*self.attachments, frame)
        loads = [
            Force(self.attachments[0], -force*relative_position/length),
            Force(self.attachments[-1], force*relative_position/length),
        ]
        return loads
...
def _point_pair_relative_position(point_1, point_2, frame=None):
    """The relative position between a pair of points."""
    
    if frame:
        return point_2.pos_from(point_1).express(frame)
    else:
        return point_2.pos_from(point_1)


def _point_pair_length(point_1, point_2, frame=None):
    """The length of the direct linear path between two points."""
    #print("Returning this for {}".format(frame))
    return _point_pair_relative_position(point_1, point_2, frame).magnitude()
    

def _point_pair_extension_velocity(point_1, point_2):
    """The extension velocity of the direct linear path between two points."""
    return _point_pair_length(point_1, point_2, frame).diff(dynamicsymbols._t)
